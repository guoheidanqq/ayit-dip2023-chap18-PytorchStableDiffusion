...
class GenPipeControlnet:
    def __init__(self,
                 vaeEncoder:nn.Module,
                 vaeDecoder:nn.Module,
                 clipEncoder:nn.Module,
                 diffusionProcessControlnet:nn.Module,
                 device = 'cuda'):
        self.diffusionProcessControlnet = diffusionProcessControlnet
        ...
        self.controlHint = None
        self.isDoingControlnet = False        
        
            
        
        # all the above provide a default value for the parameters
    
    def setControlHint(self,controlHint:torch.Tensor):
        self.controlHint = controlHint
        self.isDoingControlnet = True
    ...
    def genImage(self,prompPositive:str='a cat',
        ...
        diffusionProcessControlnet = self.diffusionProcessControlnet
        ...
        if self.controlHint is not None:
            controlHint = self.controlHint
            isDoingControlnet =  self.isDoingControlnet
        if self.controlHint is None:
            raise RuntimeError("please use setControlHint first")
        ...
        with torch.no_grad():
            ...
            for i,time in enumerate(timesteps):
                ...
                timeStep =time[None].to(device)
                modelEstimatedNoiseInLatent  = diffusionProcessControlnet(modelNoisedLatentInput,contextInput,timeStep,controlHint)
